...
class FinepSpider(scrapy.Spider):
    ...
    start_urls = [
        "http://www.finep.gov.br/chamadas-publicas/chamadaspublicas?situacao=aberta"  # noqa: E501
    ]
    ...
    def parse_details(
        ...
        description = " ".join(
            response.css("div.group.desc div.text p::text").getall()
        ).strip()
        date = response.xpath(
            "//div[contains(., 'Data de Publicação:')]"
            "/following-sibling::div[@class='text']/text()"
        ).get()
        closing_date = response.xpath(
            "//div[contains(., 'Prazo para envio de propostas até:')]"
            "/following-sibling::div[@class='text']/text()"
        ).get()
